...
curl_params=${CURL_PARAMS:-"-sS --retry 3"}

while true
do
  if [ -n "${HEALTHCHECK_START_URL}" ]; then
    curl $curl_params "${HEALTHCHECK_START_URL}"
  fi

  ip=$(curl $curl_params https://ipinfo.io/ip)

  # SCRIPT START
  echo "[Cloudflare DDNS] Check Initiated for <$record_name>"

  # Seek for the record
  record=$(curl $curl_params -X GET "https://api.cloudflare.com/client/v4/zones/$zone_identifier/dns_records?name=$record_name" -H "X-Auth-Email: $auth_email" -H "X-Auth-Key: $auth_key" -H "Content-Type: application/json")
  ...
  if [ $ip == $old_ip ]; then
    ...
    update=$(curl $curl_params -X PUT "https://api.cloudflare.com/client/v4/zones/$zone_identifier/dns_records/$record_identifier" -H "X-Auth-Email: $auth_email" -H "X-Auth-Key: $auth_key" -H "Content-Type: application/json" --data "{\"id\":\"$zone_identifier\",\"type\":\"A\",\"proxied\":$proxied,\"name\":\"$record_name\",\"content\":\"$ip\"}")
  ...
  if [ -n "${HEALTHCHECK_END_URL}" ]; then
    curl $curl_params "${HEALTHCHECK_END_URL}"
  fi
